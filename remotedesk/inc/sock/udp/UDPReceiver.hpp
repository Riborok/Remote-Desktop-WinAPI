...
private:
    ...
    static size_t extractTotalSize(const std::vector<byte>& payload, const int bytesReceived);
    static size_t extractFragmentNumber(const std::vector<byte>& payload, const int bytesReceived);
    static size_t extractId(const std::vector<byte>& payload, const int bytesReceived);
    static size_t extractSizeTFromPayload(const std::vector<byte>& payload, const size_t startIdx);
};
